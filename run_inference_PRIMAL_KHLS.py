...
from model_forward_KL_KHLS_inference import Inference_Model_KHLS_NN
...
class Inferencer(object):
    ...
    def __init__(self, config, PML_para, session,mean_p,Sigma_p,weight_p,KL_m_norm,trial_i):
        ...
        self.model = Inference_Model_KHLS_NN(self.session,PML_para,self.config,dim, n_gmm, self.config.K_r, K_b, self.config.d_v,self.config.d_w, self.config.d_y,self.config.d_z,self.config.d_mu_1, self.n_vs,self.config.n_ps_v,self.p_target)
    ...
    def train(self):
        ...
        MIN_KL=1e10
        count_bad_kl=0
        weight_hat_min = self.session.run(self.model.q_approx.weight)
        # sio.savemat('./inference_data_HLS/weight_hat_' + str(self.trial_i) + '_min.mat', {'weight_hat': weight_hat})
        mean_hat_min = self.session.run(self.model.q_approx.mean)
        # sio.savemat('./inference_data_HLS/mu_hat_' + str(self.trial_i) + '_min.mat', {'mean_hat': mean_hat})
        Prcs_hat_min = self.session.run(self.model.q_approx.Prcs)
        # sio.savemat('./inference_data_HLS/Prcs_hat_' + str(self.trial_i) + '_min.mat', {'Prcs_hat': Prcs_hat})
        q_var_min = self.session.run(self.model.q_v)
        # sio.savemat('./inference_data_HLS/q_'+str(self.trial_i)+'_min.mat', {'q': q_var})
        v_var_min = self.session.run(self.model.v)
        ...
        while it<=self.config.MAXIT:
            ...
            sio.savemat('./inference_data_HLS/q.mat', {'q': q_var})
        ...
        if it>=self.config.MAXIT:
            #weight_hat = self.session.run(self.model.q_approx.weight)
            sio.savemat('./inference_data_HLS/weight_hat_'+str(self.trial_i)+'_min.mat', {'weight_hat': weight_hat_min})
            #mean_hat = self.session.run(self.model.q_approx.mean)
            sio.savemat('./inference_data_HLS/mu_hat_'+str(self.trial_i)+'_min.mat', {'mean_hat': mean_hat_min})
            #Prcs_hat = self.session.run(self.model.q_approx.Prcs)
            sio.savemat('./inference_data_HLS/Prcs_hat_'+str(self.trial_i)+'_min.mat', {'Prcs_hat': Prcs_hat_min})
            #v_var = self.session.run(self.model.v)
            sio.savemat('./inference_data_HLS/Inference_v_'+str(self.trial_i)+'_min.mat', {'v': v_var_min})
            #q_var =self.session.run(self.model.q_v)
            sio.savemat('./inference_data_HLS/q_'+str(self.trial_i)+'_min.mat', {'q': q_var_min})
...
def main():
    import argparse
    filename='Synthetic_Data/train_and_test_primal/gmm_test'

    try:
        gmm_data = sio.loadmat(filename+'.mat')

        mean_p = np.float32(gmm_data['mu_interp'])
        print(mean_p.shape)

        Sigma_p = np.float32(gmm_data['Sigma_interp'])
        print(Sigma_p.shape)

        weight_p = np.float32(gmm_data['weight_interp'])
        ...
        mean_p = np.transpose(np.float32(gmm_data['mu_interp']), (2, 1, 0))
        print(mean_p.shape)

        Sigma_p = np.transpose(np.float32(gmm_data['Sigma_interp']), (2, 3, 1, 0))
        print(Sigma_p.shape)

        weight_p = np.transpose(np.float32(gmm_data['weight_interp']), (1, 0))
        print(weight_p.shape)
    ...
    PML_para=sio.loadmat('Server_train_data_HLS/PML_para_99_min.mat')
    ...
    parser.add_argument('--MAXIT', type=int, default=500, required=False)
    ...
    parser.add_argument('--start_update_q', type=int, default=50, required=False)
    parser.add_argument('--start_MC_q', type=int, default=100, required=False)
    parser.add_argument('--n_gmm', type=int, default=mean_p.shape[2], required=False)
    parser.add_argument('--n_vs',type=float,default=2.0,required=False)
