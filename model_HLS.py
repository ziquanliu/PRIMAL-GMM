...
class Model_HLS():
    def __init__(self, sess, config, dim,n_gmm, K_r, K_b, d_v,d_w, d_y, d_z, n_v, p_model,scope_name='inverse_KL'):
        self.p_model=p_model
        self.K_r=K_r
        self.K_b=K_b
        self.dim=dim
        ...
        with tf.variable_scope(scope_name) as scope:
            self.v=tf.get_variable('v',shape=(n_gmm,d_v),dtype=tf.float32,
                                   initializer=tf.random_uniform_initializer(0.0, 1.0))
            self.H_w=tf.get_variable('H_w',shape=(d_v,d_w),dtype=tf.float32,
                                   initializer=tf.random_uniform_initializer(0.0, 1.0))
            self.H_z=tf.get_variable('H_z',shape=(d_v,d_z),dtype=tf.float32,
                                   initializer=tf.random_uniform_initializer(0.0, 1.0))
            self.H_y=tf.get_variable('H_y',shape=(d_v,d_y),dtype=tf.float32,
                                   initializer=tf.random_uniform_initializer(0.0, 1.0))
        ...
        self.lv_train=[self.v,self.H_w,self.H_y,self.H_z]
...
class Model_HLS_diag_cov():
    def __init__(self, sess, config,dim,n_gmm, K_r, K_b, d_v,d_w, d_y, d_z, d_C_1, n_v, p_model,scope_name='inverse_KL'):
        self.p_model=p_model
        self.K_r=K_r
        self.K_b=K_b
        self.dim=dim
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            self.xi = tf.get_variable('xi', shape=(d_w, K_r), dtype=tf.float32,
                                        initializer=tf.random_uniform_initializer(0.0, 1.0))
            #print(self.w)
            self.q_approx=GaussianMixture_HLS_diag_cov(self.v,self.H_w,self.H_y,self.H_z,self.b,self.C_1,self.C_b_1,self.C_out,self.C_out_b,self.mu,self.xi,n_gmm,dim,K_r,K_b)
