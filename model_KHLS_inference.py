...
class Inference_Model_KHLS_NN():
    def __init__(self, sess, PML_para,config, dim,n_gmm, K_r, K_b, d_v,d_w, d_y, d_z, d_mu_1,n_v, n_ps_v,p_model,scope_name='inverse_KL'):
        ...
        with tf.variable_scope(scope_name) as scope:
            ...
            self.ps_v=tf.get_variable('ps_v',dtype=tf.float32,
                                      initializer=PML_para['ps_v'])
            ...
            self.mu_1=tf.get_variable('mu_1',dtype=tf.float32,
                                      initializer=PML_para['mu_1'])
            self.b_1=tf.get_variable('b_1',dtype=tf.float32,
                                     initializer=PML_para['b_1'])

            self.mu_out=tf.get_variable('mu_out',dtype=tf.float32,
                                      initializer=PML_para['mu_out'])
            self.b_out=tf.get_variable('b_out',dtype=tf.float32,
                                     initializer=PML_para['b_out'])
    ...
    def MH_sample_update(self,sample,kl_arr_val,q_swap,it):
        ...
        #v_sample_MH = tf.cond(rand_num > print_swap_prob, lambda: self.v[sample,:], lambda: tf.random.uniform(shape=(self.config.d_v,),minval=-1.0,maxval=1.0))
        # put q_sample_MH back to self.q_v
        assignment_q_sample=self.q_v[sample,:,:].assign(tf.squeeze(q_sample_MH))
        #assignment_v_sample=self.v[sample,:].assign(v_sample_MH)

        return assignment_q_sample#,assignment_v_sample
