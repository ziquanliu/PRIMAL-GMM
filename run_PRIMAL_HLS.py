...
from model_HLS import Model_HLS
...
class Trainer(object):
    ...
    def __init__(self, config, session,mean_p,Sigma_p,weight_p,KL_m_norm,trial_i):
        ...
        self.model = Model_HLS(self.session,self.config,dim, n_gmm, self.config.K_r, K_b, self.config.d_v,self.config.d_w, self.config.d_y, self.config.d_z, self.n_vs,self.p_target)
    ...
    def train(self):
            ...
            if it>=self.config.start_update_q:
                ...
                if kl_loss<MIN_KL:
                    ...
                    sio.savemat('./train_data_HLS/PML_para_' + str(self.trial_i) + '_min.mat',
                                {'v': v_var, 'H_w': H_w_var, 'H_y': H_y_var,
                                 'H_z': H_z_var, 'b': b_var, 'C': C_var, 'beta': beta_var,
                                 'mu': mu_var, 'xi': xi_var, 'q': q_var,
                                 'config': self.config})
        ...
        sio.savemat('./train_data_HLS/PML_para_'+str(self.trial_i)+'.mat', {'v': v_var ,'H_w':H_w_var, 'H_y':H_y_var,
                                                                            'H_z':H_z_var, 'b':b_var, 'C':C_var,'beta':beta_var,
                                                                            'mu':mu_var, 'xi':xi_var, 'q':q_var,'config':self.config})
...
def main():
    ...
    parser.add_argument('--MAXIT', type=int, default=500, required=False) # max EM iterations
    ...
    parser.add_argument('--K_r', type=int, default=2, required=False) # number of reconstructed components
    ...
    parser.add_argument('--d_mu_1', type=int, default=3, required=False) # dimension of the hidden layer in mean generation neural network
    parser.add_argument('--reg_v', type=float, default=0.01, required=False) # coefficient of L2 norm regularizer for v
    ...
    parser.add_argument('--T_coeff', type=float, default=0.05, required=False) # temperature parameters in updating q
    parser.add_argument('--start_update_q', type=int, default=50, required=False) # the step to start to update q
    parser.add_argument('--start_MC_q', type=int, default=100, required=False) # the step to start to MH sampling for q
